...
sample_size.all        <- list(100, 200, 400)
target.power.all       <- list(NULL)
cov.type.all           <- list("ST")
cov.par.all            <- list(quote(list("var" = 1, "cor" = nlme::corCAR1(0.5, form = ~ time | Subject))))
alloc.ratio.all        <- list(c(1,1))
mean_diff_fnm.all      <- list("mean.diff")
sigma2.e.all           <- list(0.001)
sig.level.all          <- list(0.05)
missing_type.all       <- list("constant")
missing_percent.all    <- list(0.1, 0.2, 0.4)
...
save(sim.design.all, file = "Files/sim_design_miss.Rdata")
