...
cov.type.all           <- list("NS")
cov.par.all            <- list(quote(list("cov.obj" = NULL, 
                                          "eigen.comp" = list("eig.val" = c(1, 0.5), 
                                                              "eig.obj" = function(t){
                                                                sqrt(2)*cbind(sin(2*pi*t), cos(2*pi*t))
                                                              }
                                          ) )))
...
nobs_per_subj.all      <- list(quote(4:7), quote(8:12))
...
save(sim.design.all, file = "Files/sim_design.Rdata")
